...
R_sq_OLR_redu = R2(Data_red_test$quality,OLR_check_red_reduced)
...
print(c(R_sq_OLR_redu,RMSPE_OLR_redu/sd(Data_red_test$quality),MAPE_OLR_redu))
...
Data_red_training_RF$quality = factor(Data_red_training$quality, levels = c("3","4","5","6","7","8"),ordered = TRUE)
...
#GLM 
```{r}
Data_red_test_GLM= Data_red_test
Data_red_training_GLM = Data_red_training

GLM_red = glm(formula = quality ~., data = Data_red_training_GLM) 

#Assumption check for the col-linearity 
vif(GLM_red)
X_matrix_red = as.matrix(Data_red_training_GLM[,-1])
XX_red=t(X_matrix_red)%*%X_matrix_red
lambda_red = eigen(XX_red)$values
cond_number_red=max(lambda_red)/min(lambda_red)
indices_red=max(lambda_red)/lambda_red

plot(GLM_red)


#Prediction; Model check 
GLM_check_red = round(as.matrix(predict(GLM_red,Data_red_test_GLM)),0)
paste("The accurate rate for GLM is ",round(Check_accurate_rate(Data_red_test,GLM_check_red),5))

R_sq_GLM_red = R2(Data_red_test$quality,GLM_check_red)
RMSPE_GLM_red = RMSE(Data_red_test$quality,GLM_check_red)
MAPE_GLM_red = MAE(Data_red_test$quality,GLM_check_red)
print(c(R_sq_GLM_red,RMSPE_GLM_red/sd(Data_red_test$quality),MAPE_GLM__red))

table(t(Data_red_test[,1]),GLM_check_red)
...
#Clustering
#Unsupervised statistic learning is used on the data where no response variables
